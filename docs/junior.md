...
开发环境：ssh 连接类脑平台(连接方法见[lecture-4](./course.md#lecture-4-environment-setup))

提交窗口：希冀平台(cscourse.ustc.edu.cn, 初始用户名为学号，密码为学号)
...
本赛题要求选⼿在 Ascend 910B3 硬件平台上，**使⽤ Triton-Ascend 或 TileLang-Ascend 语言**（二选一即可）编写 `Softmax` 算⼦。具体题面见 [Assignment-1 PDF](./assignment/softmax.pdf)。
